window.onload = function () {
    ...
    body.addEventListener('click', (e) =>{
        ...
        if (e.target.id === "check_btn" && e.target.parentNode.id === 'pop') {
            ...
            if (form_left[0].children[0].value === "" || form_left[2].children[0].value === "" || form_left[5].value === "") {
                form_left[0].children[0].setAttribute('class', 'change')
                form_left[2].children[0].setAttribute('class', 'change')
                form_left[5].setAttribute('class', 'change')
            } else if ( form_left[6].children[0].value === "") {
                form_left[6].children[0].style.border = '1px solid #f23030'
                setTimeout(function(){
                    form_left[6].children[0].style.border = '1px solid #648F84'
                }, 3000)
        ...
        if (e.target.id === 'delet_btn') {
            const lend_id = e.target.parentElement.classList[1]
            deleteHeadler(lend_id)
        }
    })
    tBody.addEventListener('click', function (e) {
        ...
        if (e.target.className === "fa-close icon") {
            ...
            const lend_id = tr.parentElement.nextElementSibling.classList[1]
            close_openTrs()
            closeIcon.classList.add('activ');
            tr.parentElement.setAttribute('class', 'bg_blue');
            document.getElementsByClassName(lend_id)[0].style.display = 'inline-block';
            delet_list_pop(lend_id)
        }
    })
    ...
    BindValue(listArray, 'lists', function (val) {
        val.forEach(item => {
            ...
            let detailTemplate = `
            <tr class="detail ${item.lend_id} ${item.source_warehouse_id}" >
                ...
                <td>借出單號<input type="text" value="${item.lend_id}" name="return_id" readonly="readonly"></td>
                <td>銷貨單據號碼<input put type="text" value="${item.order_number}" name="order_number" readonly="readonly"></td>
                <td>出庫日期<input type="text" value="${item.lend_date}" name="return_date" readonly="readonly"></td>
                <td>棧板編號<div></div></td>
                <td>棧板數量總計</td>
            ...
            document.getElementsByClassName(item.lend_id)[0].style.display = 'none'
            palletIdHandler(item.pallet, item.lend_id)
        });

        function palletIdHandler(palletArray, lend_id) {
            const detailNodeList = document.getElementsByClassName(lend_id)[0].children
            const pallet_div = detailNodeList[5].children[0]
            const pallet_total = detailNodeList[6]
    ...
    //刪除借出單
    function deleteHeadler(lend_id) {
        const obj = {
            "lend_id": lend_id,
            "program_id": program_id,
        ...
        axios.put(lend_dataChange_url, obj ,{
            headers: {'Authorization': token}
        })
            .then(res => {
                console.log(res)
    ...
    function postHandler(form_left, postArray) {
        const obj = {
            ...
            "order_number": form_left[5].value,
            "lend_date": form_left[6].children[0].value,
            "remark": form_left[7].children[0].value,
            "creator_user": userId,
            "account_number": userId,
            "pallet_lend_detail": postArray
        }
        axios.post(lend_dataChange_url, obj, {
            headers: {'Authorization': token}
        })
        .then(res => {
            console.log(res)
